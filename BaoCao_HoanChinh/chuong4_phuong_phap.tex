...
\begin{tikzpicture}[node distance=1.5cm]
    % Nodes
    \node (input) [startstop] {Video Input / Camera};
    
    % Nhanh 1: Xe va Tracking
    \node (yolo) [process, below of=input] {YOLOv8 Detect \& ByteTrack};
    \node (tracker) [process, below of=yolo] {Vehicle Tracker (Pos History)};
    
    % Nhanh 2: Tinh Huong (Phuc tap hon)
    \node (traj) [process, below of=tracker, xshift=-2.5cm] {Trajectory Analysis (Ref Vector)};
    \node (roi_dir) [process, below of=tracker, xshift=2.5cm] {ROI-based Direction};
    \node (fusion) [decision, below of=tracker, yshift=-2.5cm] {Direction Fusion};
    
    % Nhanh 3: Den tin hieu
    \node (tl_man) [process, right of=yolo, xshift=5cm] {TL Manager (ROI \& HSV)};
    
    % Logic Vi pham
    \node (stopline) [process, right of=fusion, xshift=3cm] {Stopline Manager};
    \node (engine) [process, below of=fusion, yshift=-1cm] {Violation Engine};
    
    % ALPR Branch
    \node (alpr) [process, left of=engine, xshift=-3cm] {ALPR Pipeline};
    
    \node (output) [startstop, below of=engine] {Cảnh báo \& Thống kê};

    % Arrows
    \draw [arrow] (input) -- (yolo);
    \draw [arrow] (yolo) -- (tracker);
    \draw [arrow] (tracker) -- (traj);
    \draw [arrow] (tracker) -- (roi_dir);
    \draw [arrow] (traj) |- (fusion);
    \draw [arrow] (roi_dir) |- (fusion);
    
    \draw [arrow] (input) -| (tl_man);
    \draw [arrow] (tl_man) |- (engine);
    
    \draw [arrow] (fusion) -- (engine);
    \draw [arrow] (stopline) -- (engine);
    \draw [arrow] (tracker) -| (alpr);
    \draw [arrow] (alpr) |- (output);
