...
\begin{tikzpicture}[
    ...
    decision/.style={diamond, draw, minimum width=2cm, minimum height=1cm, align=center, font=\small},
    ...
    \node[box, fill=orange!20, below=0.8cm of input] (yolo) {YOLOv8\\Detect};
    \node[box, fill=yellow!20, below=0.8cm of yolo] (track) {ByteTrack\\Tracking};
    
    % Split into 3 branches
    \node[box, fill=green!20, below left=1.2cm and 2cm of track] (veh_tracker) {Vehicle\\Tracker};
    \node[box, fill=cyan!20, below=1.2cm of track] (tl_manager) {Traffic Light\\Manager};
    \node[box, fill=pink!20, below right=1.2cm and 2cm of track] (alpr) {ALPR\\Pipeline};
    
    % Vehicle Tracker sub-branches
    \node[box, fill=green!30, below left=1cm and 0.8cm of veh_tracker] (traj) {Trajectory\\Analyzer};
    \node[box, fill=green!30, below right=1cm and 0.8cm of veh_tracker] (roi_dir) {ROI\\Direction};
    \node[decision, fill=green!40, below=2cm of veh_tracker] (fusion) {Direction\\Fusion};
    
    % ALPR sub-pipeline
    \node[box, fill=pink!30, below=0.8cm of alpr] (ocr) {PaddleOCR};
    \node[box, fill=pink!30, below=0.8cm of ocr] (validate) {Validation\\& Retry};
    
    % Violation Detection
    \node[box, fill=red!20, below=3cm of track] (stopline) {Stopline\\Manager};
    \node[box, fill=red!30, below=1.5cm of stopline] (violation) {Violation\\Engine};
    
    % Output
    \node[box, fill=purple!20, below=0.8cm of violation] (output) {Alert \&\\Statistics};
    ...
    % To Violation Engine
    \draw[arrow] (fusion) |- (violation);
    \draw[arrow] (tl_manager) -| (violation);
    \draw[arrow] (stopline) -- (violation);
    \draw[arrow] (validate) |- (violation);
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{../BÁO_CÁO_ADVC/Figures/labels.jpg}
