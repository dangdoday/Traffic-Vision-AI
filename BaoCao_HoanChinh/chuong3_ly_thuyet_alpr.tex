...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[box, right=0.5cm of pred] (output) {``30G07784''};
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \node at (4,0.75) {\textbf{3 0 G 0 7 7 8 4}};
        
        % Vertical slices
        \foreach \x in {0,1,...,8} {
            \draw[dashed, gray] (\x,0) -- (\x,1.5);
        }
        
        % Labels
        \node at (4,-0.5) {Ảnh biển số được chia thành các ``lát cắt'' dọc};
        
        % Feature vectors below
        \foreach \x in {0.5,1.5,...,7.5} {
            \draw[->] (\x,0) -- (\x,-0.3);
        }
        \node at (4,-1) {$\downarrow$ mỗi lát $\rightarrow$ vector đặc trưng $D$ chiều};
    \end{tikzpicture}
...
\textbf{Ví dụ:} Với ảnh biển số ``30G'', output của mô hình có thể là:
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
Lát & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\ \hline
Output & 3 & 3 & $\epsilon$ & 0 & $\epsilon$ & G & G \\ \hline
\end{tabular}
\end{center}

Sau khi áp dụng CTC collapse (loại bỏ lặp và blank): ``33-0-GG'' $\rightarrow$ ``30G''
...
\begin{itemize}
    ...
    \item $\mathbf{l}$: Chuỗi ký tự mục tiêu (VD: ``30G07784'')
    \item $\pi$: Một alignment path (VD: ``3-3-0-0-G-G-0-...'')
...
%============================================================
% SECTION MỚI: HAI PHƯƠNG PHÁP TRIỂN KHAI ALPR
%============================================================
