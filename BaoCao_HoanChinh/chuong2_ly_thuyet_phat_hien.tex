...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../BÁO_CÁO_ADVC/Figures/chia_luoi.png}
...
\begin{itemize}
    \item \textbf{YOLOv1-v3:} Tập trung vào tốc độ thời gian thực, giới thiệu khái niệm Anchor Box để ổn định việc huấn luyện.
...
Một trong những cải tiến quan trọng nhất của YOLOv8 là loại bỏ cơ chế gán nhãn dựa trên IoU tĩnh (static IoU-based assignment) và thay thế bằng \textbf{Task Aligned Assigner}. Chiến lược này chọn lọc các mẫu dương (positive samples) dựa trên sự cân bằng giữa điểm phân loại và độ chính xác định vị.
...
Ngoài ra, YOLOv8 còn kết hợp \textbf{Distribution Focal Loss (DFL)} để tối ưu hóa phân phối xác suất của các cạnh bounding box, giúp mô hình hội tụ nhanh hơn khi dự đoán các box có biên mờ.
...
\section{Các độ đo đánh giá (Evaluation Metrics)}
...
\end{equation}
Trong đó: $TP$ (True Positive) là số detection đúng (IoU $>$ ngưỡng), $FP$ (False Positive) là detection sai, $FN$ (False Negative) là đối tượng bị bỏ sót.
...
\end{equation}
Chỉ số \textbf{mAP@50} (tính tại ngưỡng IoU=0.5) và \textbf{mAP@50:95} (trung bình trên các ngưỡng IoU từ 0.5 đến 0.95) được sử dụng làm tiêu chí chính để so sánh các mô hình trong báo cáo này.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../BÁO_CÁO_ADVC/Figures/bytetrack.png}
...
\begin{enumerate}
    ...
    \item \textbf{Tốc độ thực thi cao:} ByteTrack chỉ sử dụng thông tin hình học (geometric information) để liên kết mà không cần trích xuất đặc trưng ngoại hình (appearance features) bằng mạng nơ-ron sâu như DeepSORT. Điều này giúp giảm tải tính toán cho GPU, đảm bảo hệ thống duy trì được FPS cao (frames per second).
\end{enumerate}
...
Làn đường là đa giác (không bắt buộc lồi) \(P = \{v_1,\dots,v_n\}\). Tâm xe \(C(x,y)\) được kiểm tra bằng hàm \texttt{cv2.pointPolygonTest} (tương đương ray casting): kết quả \(\ge 0\) nếu điểm nằm trong/biên, \(<0\) nếu ngoài.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[scale=1.6]
    ...
    \draw[->,thick,green!70!black] (O) -- ++(90:2.2) node[above] {$\vec r$ (ref - đi thẳng)};
