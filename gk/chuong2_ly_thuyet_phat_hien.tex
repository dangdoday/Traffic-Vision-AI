...
\subsubsection{Thuật toán Hungarian (Kuhn--Munkres)}
Thuật toán Hungarian giải bài toán phân công tuyến tính bằng cách tìm một phép ghép 1-1 tối ưu giữa tập track và detection trên ma trận chi phí $C \in \mathbb{R}^{n \times n}$. Nếu ma trận không vuông ($M \neq N$) thì bổ sung hàng/cột giả để đưa về kích thước $n = \max(M, N)$; nếu cần cực đại hóa, chuyển sang cực tiểu hóa bằng $C' = C_{\max} - C$ hoặc dùng $-C$. \cite{kuhn1955hungarian}

Quy trình chuẩn của thuật toán gồm các bước sau:
\begin{enumerate}
    \item \textbf{Giảm hàng}: trừ phần tử nhỏ nhất của mỗi hàng cho toàn bộ hàng.
    \item \textbf{Giảm cột}: trừ phần tử nhỏ nhất của mỗi cột cho toàn bộ cột.
    \item \textbf{Phủ các số 0}: dùng số đường thẳng tối thiểu (ngang/dọc) để phủ hết các số 0 trong ma trận.
    \item Nếu số đường phủ bằng $n$, ta chọn tập số 0 sao cho mỗi hàng/cột chỉ chọn đúng một phần tử (tương ứng với phép gán tối ưu). Nếu chưa đủ, tìm phần tử nhỏ nhất chưa bị phủ là $m$, trừ $m$ cho mọi phần tử chưa phủ và cộng $m$ cho các phần tử ở giao điểm của hai đường phủ; sau đó lặp lại bước 3.
\end{enumerate}

Kết quả là ánh xạ ghép cặp tối ưu toàn cục với độ phức tạp $O(n^3)$, phù hợp cho bài toán theo dõi thời gian thực.\subsection{Thuật toán ByteTrack}
