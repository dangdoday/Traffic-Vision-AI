...
\begin{frame}{Validation \& Character Correction}
    \begin{columns}[c]
    \begin{column}{0.55\textwidth}
        \begin{block}{Format Validation (Biển số Việt Nam)}
        \begin{itemize}
            \item \textbf{Ô tô:} \texttt{[0-9]\{2\}[A-Z][0-9]\{4,5\}}
            \item \textbf{Xe máy:} \texttt{[0-9]\{2\}[A-Z][A-Z0-9][0-9]\{4,5\}}
        \end{itemize}
        \end{block}
        
        \begin{block}{Character Correction}
        Sửa lỗi nhầm lẫn ký tự dựa trên vị trí:
        \begin{itemize}
            \item Vị trí 0-1: \texttt{O} $\to$ \texttt{0}
            \item Vị trí 2: \texttt{0} $\to$ \texttt{O}
            \item Vị trí 3+: \texttt{B} $\to$ \texttt{8}
        \end{itemize}
        \end{block}
    \end{column}
    \begin{column}{0.42\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Figures/plate_val_pred.jpg}
        
        \tiny Minh họa kiểm tra định dạng
    \end{column}
    \end{columns}
\end{frame}
